...
pub mod prod {
    ...
    // NOTE: the Postmark API base is the host (no path). The client appends `/email` when
    // building the request URL. Having `/email` here would result in a duplicated
    // `/email/email` path which causes requests to go to the wrong endpoint.
